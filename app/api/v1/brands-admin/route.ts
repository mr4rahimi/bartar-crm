...
import { createBrandService } from '@/features/pricing/services/catalog.service';
...
export async function POST(request: NextRequest) {
  try {
    ...
    return apiSuccess(await createBrandService(parsed.data.name, context), 201);
  } catch (error) {
    return handleApiError(error, '[api/v1/brands-admin POST]');
  }
}
