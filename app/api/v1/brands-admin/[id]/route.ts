...
import { updateBrandService, deleteBrandService } from '@/features/pricing/services/catalog.service';
...
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    return apiSuccess(await updateBrandService(params.id, parsed.data.name, context));
  } catch (error) {
    return handleApiError(error, '[api/v1/brands-admin/:id PATCH]');
  }
}
...
export async function DELETE(request: NextRequest, { params }: Params) {
  try {
    const context = await authorizeCatalog(request);
    await deleteBrandService(params.id, context);
    return apiSuccess(null);
  } catch (error) {
    return handleApiError(error, '[api/v1/brands-admin/:id DELETE]');
  }
}
